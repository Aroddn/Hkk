...
public class CollectionBrowser : MonoBehaviour {
    ...
    {
        ShowCards(_showingCardsPlayerDoesNotOwn, _pageIndex, _includeAllRarities, _includeAllCharacters, _rarity, _asset,_set, _keyword, _manaCost, _includeTokenCards);
    }
        ...
        RarityOptions rarity = RarityOptions.ALL, CharacterAsset asset = null, Set set = Set.All, string keyword = "", int manaCost = -1, bool includeTokenCards = false)
    {
        ...
        List<CardAsset> CardsOnThisPage = PageSelection(showingCardsPlayerDoesNotOwn, pageIndex, includeAllRarities, includeAllCharacters, rarity,
            asset, set, keyword, manaCost, includeTokenCards);
    ...
    public void Next()
    {
        if (PageSelection(_showingCardsPlayerDoesNotOwn, _pageIndex+1,_includeAllRarities, _includeAllCharacters, _rarity,
            _asset, _set, _keyword,_manaCost, _includeTokenCards).Count == 0)
            return;
        
        ShowCards(_showingCardsPlayerDoesNotOwn, _pageIndex+1,_includeAllRarities, _includeAllCharacters, _rarity,
            _asset,_set,_keyword,_manaCost, _includeTokenCards);
    }
